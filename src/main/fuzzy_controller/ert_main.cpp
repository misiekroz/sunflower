...
#include "codegen_simulink.h"          // Model header file
...
void rt_OneStep(void)
{
  ...
  // Check for overrun
  if (OverrunFlag) {
    rtmSetErrorStatus(codegen_simulink_M, "Overrun");
    return;
  }
...
int_T main(int_T argc, const char *argv[])
{
  ...
  fflush((nullptr));
  while (rtmGetErrorStatus(codegen_simulink_M) == (nullptr)) {
    //  Perform application tasks here
  }

  // Terminate model
  codegen_simulink_terminate();
  return 0;
}
